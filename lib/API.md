...
- `parse_request_head(buf, len, ParsedHead *)`, `request_head_is_complete(ParsedHead *, buf, len, ChunkScanState *)` (P8: last arg resumes a chunked-body scan across reads of one request, `NULL` = from scratch), `parse_http_request_from_head(raw, raw_len, ParsedHead *, Request *, Arena *)` (P2: one `phr_parse_request` pass, reused by `connection.c`'s body-limit check, completeness check and full parse instead of each running its own; `request_framing`/`request_is_complete`/`parse_http_request` are thin wrappers over these and unchanged in behavior).
