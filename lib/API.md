...
- Static files (`static.h`): `static_serve_file`, `static_resolve_relative_path`, `static_mime_type`, `static_cache_clear` (clears `static_serve_file`'s in-memory file cache; tests and app-triggered reloads only, nothing in the engine calls it).
