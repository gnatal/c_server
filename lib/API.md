...
## Wait on I/O without blocking the worker (`connection.h`; recipe 15)
- `res_defer(res)` → `DeferHandle` — the response comes later: return from the handler, keep the handle (never `res`/`req`). What the handler set or allocated before stays valid. `0` = refused: 503 already built (over `max_buffered_bytes`), or nothing changed (already answered/deferred). Requests pipelined behind it wait; a client hang-up closes it; 504 + close after `DEFER_TIMEOUT_SECONDS` (30). Make a `yyjson_alc` after `res_resume`, not before `res_defer`.
- `res_resume(app, h)` → `Response *` to fill with any `res_*`, written right after the current callback returns (safe from another handler or a watch callback); `NULL` when the request is gone (client left, 504, shutdown, already answered): drop the result. Resumed with nothing sent → 500.
...
- `response_clone_used(dst, src)` — engine: copy only the used slots of a Response (header/trailer values stay pointers into the arena that built them).
...
- `arena_grow(Arena *, ptr, old_size, new_size)` — resize an arena block (keeps its bytes): in place when it is the last block in the buffer, `realloc` when it is the newest fallback block, else allocate + copy. `NULL` on failure, `ptr` still valid. Used by yyjson's realloc hook and `res_write`.
...
- `arena_hand_over(from, to, fresh_buf, cap)` — engine: move everything allocated in `from` into `to` without copying, restart `from` on `fresh_buf`. `Arena.large_bytes` counts the fallback blocks' bytes.
- `arena_pool_init(ArenaPool *, block_size, max_spare)`, `arena_pool_take`, `arena_pool_give`, `arena_pool_destroy`, `arena_release_to_pool(Arena *, ArenaPool *)` — engine: free list of equal-size arena buffers (`App.arena_pool`, `ARENA_SIZE` blocks, at most `ARENA_POOL_MAX_SPARE` kept).
...
- `parse_http_request(raw, raw_len, Request *, Arena *)`, `request_is_complete(buf, len)`, `request_framing(buf, len, &header_len, &chunked, &path, &path_len)` (`path`/`path_len` out params optional, pass `NULL`), `request_wants_close(req)`, `request_clone_used(dst, src, old_base, old_len, new_base, Arena *)` (copy the used fields of a Request; views into `[old_base, old_base + old_len)` are moved to `new_base`).
