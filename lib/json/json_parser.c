...
static int parse_hex4(JsonParser *p, unsigned int *out) {
  ...
  for (int i = 0; i < 4; i++) {
    const int digit = json_hex_value(advance(p));
    if (digit < 0) {
      return 0;
    }
...
static JsonValue *parse_array(JsonParser *p) {
  ...
  for (;;) {
    ...
    if (value->as.array.count == capacity) {
      ...
      JsonValue **grown =
          realloc(value->as.array.items, capacity * sizeof(JsonValue *));
      if (grown == NULL) {
        set_error(p, "out of memory");
        json_free(item);
        json_free(value);
...
static JsonValue *parse_object(JsonParser *p) {
  ...
  for (;;) {
    ...
    if (value->as.object.count == capacity) {
      capacity = (capacity == 0) ? 4 : (capacity * 2);
      JsonMember *grown =
          realloc(value->as.object.members, capacity * sizeof(JsonMember));
      if (grown == NULL) {
        set_error(p, "out of memory");
        free(key);
...
JsonValue *json_parse(const char *input, char *err, size_t err_size) {
  JsonParser parser = {
      .input = input, .pos = 0, .len = strlen(input), .error = {0}};
  ...
  if (value == NULL && err != NULL && err_size > 0) {
    snprintf(err, err_size, "%s",
             parser.error[0] != '\0' ? parser.error : "unknown parse error");
  }
  return value;
}
