 ...
 * - If num_workers > 1: verifies the port is bindable once (create_server_socket; it returns -1 on
 *   failure rather than exiting itself, and cluster_listen is what turns that into a clear error
 ...
 *   hashing balances them across workers). On macOS/BSD (CEXPRESS_SINGLE_ACCEPTOR - SO_REUSEPORT
 ...
 *   exit(EXIT_FAILURE) instead of forking forever (see lib/CLAUDE.md, "Behavior reference,
