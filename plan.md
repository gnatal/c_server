...
- **Large architectural items** (#2, #7, #8, #11): #2 (Streaming & chunked responses) is Done! Only #7 (epoll/Linux portability), #8 (multi-worker concurrency), and #11 (TLS/HTTPS) remain.
...
7. The Big Three Remaining Milestones:
   - **Linux `epoll` Portability (#7)**: Native Linux & Docker support without third-party wrappers.
   - **Multi-Worker Concurrency (#8)**: `SO_REUSEPORT` process/thread worker pools to scale across all CPU cores.
