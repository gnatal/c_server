...
- `docker_stress_test.sh`: the same idea inside Docker, to exercise the Linux io_uring backend from a Mac. Builds the image (`Dockerfile`), starts the server on a private Docker network with `--security-opt seccomp=unconfined --ulimit memlock=-1:-1` (io_uring-related flags the script chose; whether the default Docker profile blocks io_uring was not tested here), and runs a containerised `wrk` against it so the numbers do not go through Docker Desktop's localhost proxy.
